...
_COLLECT_STAGE_STRONG_ICT_MARKERS = [
    ...
    '5g', '6g', 'lte', 'ran', 'o-ran', 'open ran', 'ai-ran', '주파수',
    # 2026-07-23: 바레 '전파'는 "K라이프스타일 전파"처럼 확산·보급의 뜻으로도
    # 흔히 쓰여, 이 마커 하나로 강한-ICT 예외(뒤 비ICT 검사 스킵)가 걸리는
    # 오탐의 원인이었음(CJ 태권도 기사가 전파네트워크표준단으로 오분류·선별됨).
    # 전파(radio wave) 도메인과 결합된 복합어만 인정하도록 교체 — 진짜 전파
    # 관련 기사는 아래 복합어나 '주파수'/'무선' 등 다른 마커와 대개 함께 쓰인다.
    '전파법', '전파관리', '전파자원', '전파진흥', '전파환경', '전파규제',
    '전파산업', '전파감시', '전파인증', '전파적합성', '전파간섭', '전파신호',
    '전파망원경', '전파고도계', '전파관제', '전파연구', '전파정책',
...
_NON_ICT_COLLECT_PATTERNS = {
    ...
    'culture_entertainment': re.compile(
        ...
        r'감독|배우|드라마|콘텐츠과|기상캐스터|쇼핑호스트|아나운서.*화제|'
        r'옷입고|패션.*화제|'
        # 2026-07-31: K팝 음악방송 순위 뉴스(엠카·뮤직뱅크 등) — "음악방송"의
        # '방송'이 강한 마커는 아니라 걸리진 않았지만 커버리지 자체가 없었음.
        r'엠카|음악방송|뮤직뱅크|인기가요|쇼챔피언|가온차트|'
        # 2026-08-05: SNS 라이브 방송 중 개인 사건사고 뉴스 — "라이브 방송"이
        # PR #47의 마케팅 패턴(매출·완판·적중 동반)엔 안 걸리는 순수 커버리지
        # 공백(재현: "한국 거주 일본인 인플루언서, SNS 라이브 방송 중 사망").
        r'라이브\s*방송\s*중\s*(사망|사고|논란)',
        re.IGNORECASE,
    ),
    ...
    'marketing_promo': re.compile(
        ...
        # 2026-07-31: 라이브커머스 매출 홍보 기사 — 기존 "홈쇼핑" 패턴(하드
        # 정크)이 안 걸리는 "라이브 방송" 표현의 온라인몰 판매 실적 홍보.
        # "라이브 방송"을 앞에 요구해 정상 ICT 라이브커머스 플랫폼 기사
        # (예: "라이브 커머스" 표기)와는 겹치지 않게 좁게 설계.
        r'역시즌\s*세일|라이브\s*방송.{0,15}(매출|완판|적중)',
        re.IGNORECASE,
    ),
    ...
    'geopolitics_trade': re.compile(
        r'전쟁|분쟁|침공|교전|포격|휴전|정전|전시 상황|난민|격추|공습|'
        r'드론 공격|무인기 공격|미사일 발사|'
...
_PRIORITY_NON_ICT_PATTERNS = {
    ...
    'broadcast_industry_rating': re.compile(
        r'방송평가|재허가·?재승인|시청률|방송사업자.*평가|지상파.*순위',
        re.IGNORECASE,
    ),
    'culture_export_promo': re.compile(
        r"(라이프스타일|K-?콘텐츠|K-?푸드|K-?팝|한류|K-?컬처|K-?뷰티)[\s'\"'']{0,3}전파|"
        # 2026-08-05: "K-가스안전 전파"처럼 문화 외 산업 분야까지 "K-OOO 전파"
        # (한국식 표준/제도의 해외 확산) 패턴이 반복돼 문화 키워드 나열 대신
        # 일반화(재현: "몽골에 K-가스안전 전파… 민관 합동 가스 산업 '영토 확장'").
        r"K-[가-힣]+\s*전파",
        re.IGNORECASE,
    ),
    # 2026-07-28 추가: 시상식·방송 콘텐츠 기사가 '주파수'·'안테나'·'송신'을
    # 화제성/인기의 비유로 쓰는 관용구 — "다섯 주파수", "청룡 안테나"류는 실제
    # 무선 주파수와 무관. '주파수'가 바레 강한 마커라 우선순위 승격 필요
    # (재현: "[제5회 BSA] '꿀잼' 송신한 다섯 주파수…'청룡 안테나'가 향할 작품은?").
    'entertainment_metaphor': re.compile(
        r'(시상식|어워즈|연기대상|가요대상|청룡|백상|BSA).{0,20}(주파수|안테나|송신)|'
        r'(주파수|안테나|송신).{0,20}(시상식|어워즈|청룡|백상|작품은)',
        re.IGNORECASE,
    ),
    # 2026-07-28 추가: 제품 출시 예고 기사가 규제 인증(전파인증 등) 완료 소식을
    # 곁들이는 경우 — 표준화 정책이 아니라 순수 마케팅성 홍보. '전파인증'이
    # 바레 강한 마커라 우선순위 승격 필요(재현: "삼성, 갤럭시 탭 S12·S26 FE
    # 출격 임박…국내 전파인증 완료").
    'product_launch_hype': re.compile(
        r'출격 임박|출시 임박|공개 임박|베일 벗나|베일 벗는다',
        re.IGNORECASE,
    ),
    # 2026-08-05: 탐사보도가 위성사진을 세무조사·탈세 증거로 인용하는 경우 —
    # '위성사진'의 '위성'이 바레 강한 마커라 우선순위 승격 필요(재현: "[세미콜론]
    # 위성사진에 굴착기는 없었다…확인 한 번에 '법인세 수억원").
    'satellite_imagery_investigation': re.compile(
        r'위성사진.{0,30}(법인세|탈세|세무조사|국세청|과세|증여세|상속세)|'
        r'(법인세|탈세|세무조사|국세청)[\s\S]{0,30}위성사진',
        re.IGNORECASE,
    ),
    # 2026-08-05: 종목 추천·ETF 기사가 '위성통신'을 섹터명으로만 인용 — 실제
    # 기술·정책 내용 없이 투자 관점만 다룸. '위성'/'통신'이 바레 강한 마커라
    # 우선순위 승격 필요(재현: "주춤한 우주ETF … 돈 되는 위성통신株로 확장").
    'stock_sector_etf_play': re.compile(
        r'ETF.{0,20}(위성|우주|통신)|(위성|우주|통신)[\s\S]{0,20}ETF|'
        r'(위성|우주|통신)[가-힣]*株',
        re.IGNORECASE,
    ),
}
...
5: 해외 주요국 정책·규제 변화 — 미국 FCC·NTIA·상무부, 유럽 EC·ETSI, 일본 총무성, 중국
   공업정보화부 등의 발표, ICT 법안 통과·시행, 주파수 정책 결정, 사업자 인가, 수출통제·관세 등
   국내 ICT 산업에 영향을 줄 수 있는 미·중·EU 규제 동향
...
2: 산업계 핵심 동향 — 국내(삼성·LG·SKT·KT 등)뿐 아니라 구글·MS·엔비디아·아마존·오픈AI·애플·
