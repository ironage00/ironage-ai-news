...
### Fixed
- **주간 트렌드 리포트 핵심 이슈에서 위성통신·6G 등 특정 단 주제가 통째로
  누락되는 회귀**: 사용자가 실제 발송된 주간 리포트를 확인하다 발견 —
  전파네트워크표준단은 이번 주 내내(6G·위성통신 관련 기사 매일 보도) 정상
  수집됐는데도 주간 "핵심 이슈 TOP 5"·"급등 키워드"에 전혀 등장하지 않았음.
  - 근본 원인: `analyze_weekly_trends()`가 GPT-4o에 분석 스니펫을 붙일
    기사를 고를 때 `_stratified_daily_sample()`(날짜별 상한 30건)만 썼는데,
    이는 날짜 편중만 막을 뿐 단(unit) 편중은 막지 못함 — 뉴스 절대량이 큰
    AI융합표준단이 같은 날짜 상한 안에서도 표본을 독점해, 절대량이 적은
    전파네트워크표준단 기사가 상한 밖으로 밀려나 분석 깊이를 전혀 받지
    못했음(제목 목록 자체엔 남아있었지만 GPT-4o가 이슈로 구성하기엔
    맥락이 부족했던 것으로 추정).
  - 수정: `_unit_balanced_enriched_ids()` 신규 — 4개 단 각각에서 최근순
    20건을 먼저 확보한 뒤, 남는 슬롯만 기존 `_stratified_daily_sample()`로
    채우도록 변경. 제목 전체 나열(`_build_news_summaries`)은 기존과 동일.
  - `tests/test_trend_analyzer.py`에 실제 회귀 재현 케이스(같은 날 안에서
    절대량 큰 단이 소수 단을 완전히 밀어내는 시나리오) 포함 7건 추가.

## [0.4.24.0] - 2026-08-05

### Fixed
- **K-산업 확산 홍보·위성사진 탐사보도·SNS 사건사고·종목 ETF 커버리지 공백/
  동음이의어 4건**: 전파네트워크표준단 뉴스레터에서 사용자가 발견해 신고.
  2건은 강한 마커 충돌, 2건은 순수 커버리지 공백.
  - `culture_export_promo`에 `K-[가-힣]+\s*전파` 일반화 추가: "몽골에
    K-가스안전 전파… 민관 합동 가스 산업 '영토 확장'" — 기존엔 라이프스타일·
    콘텐츠·푸드 등 문화 키워드만 나열돼 있어 가스안전처럼 문화 외 산업의
    "K-OOO 전파"(한국식 표준·제도의 해외 확산)를 못 잡았음.
  - `_PRIORITY_NON_ICT_PATTERNS`에 `satellite_imagery_investigation` 신규:
    "[세미콜론] 위성사진에 굴착기는 없었다…확인 한 번에 '법인세 수억원" —
    탐사보도가 위성사진을 세무조사·탈세 증거로 인용, '위성사진'의 '위성'이
    바레 강한 마커라 우선순위 승격 필요.
  - `_PRIORITY_NON_ICT_PATTERNS`에 `stock_sector_etf_play` 신규: "주춤한
    우주ETF … 돈 되는 위성통신株로 확장" — 종목 추천·ETF 기사가 위성통신을
    섹터명으로만 인용, '위성'/'통신'이 바레 강한 마커라 우선순위 승격 필요.
  - `culture_entertainment`에 `라이브\s*방송\s*중\s*(사망|사고|논란)` 추가:
    "한국 거주 일본인 인플루언서, SNS 라이브 방송 중 사망" — PR #47의
    마케팅 패턴(매출·완판·적중 동반)엔 안 걸리는 순수 커버리지 공백.
  - `tests/test_collect_filter.py`에 재현 4건 + 정상 위성통신·전파·세무조사
    기사 회귀 방지 6건 추가.
...
### Fixed
- **라이브커머스 매출 홍보·K팝 음악방송 순위 커버리지 공백 2건**: 사용자가
  뉴스레터 후보에서 직접 발견해 신고. 둘 다 강한 마커 충돌이 아니라('방송'은
  애초에 강한 마커가 아님) 순수 커버리지 공백.
  - `marketing_promo`에 `역시즌 세일|라이브 방송(0~15자).(매출|완판|적중)`
    추가: "LF몰, 한여름 '역시즌 세일' 적중… 라이브 방송 역대 최고 매출 달성" —
    기존 홈쇼핑 하드정크 패턴(완판·주문액·매진 행렬)과 안 겹치는 "라이브
    방송" 표현의 온라인몰 판매 실적 홍보. "라이브 방송"을 앞에 요구해
    "라이브 커머스"(플랫폼·기술 기사에 흔한 표현, 예: SKT 라이브커머스
    매출 성장 기사)와는 겹치지 않게 좁게 설계.
  - `culture_entertainment`에 `엠카|음악방송|뮤직뱅크|인기가요|쇼챔피언|
    가온차트` 추가: "제니, '레스 댄 어 러버'로 '엠카' 1위⋯방송 출연 없이
    음악방송 1위" — K팝 음악방송 차트 순위 뉴스 장르 자체가 커버리지에
    없었음.
  - `tests/test_collect_filter.py`에 재현 2건 + 라이브커머스 정상 ICT
    회귀 방지 2건 추가, 전체 테스트 통과.
...
### Fixed
- **시상식 은유·제품 마케팅·군사 용어 커버리지 공백/동음이의어 4건**: 사용자가
  뉴스레터 후보에서 직접 발견해 신고. 2건은 강한 마커 충돌(0.4.17.0/0.4.18.0과
  같은 유형), 2건은 순수 커버리지 공백.
  - `entertainment_metaphor`(우선순위 패턴 신규): "[제5회 BSA] '꿀잼' 송신한
    다섯 주파수…'청룡 안테나'가 향할 작품은?" — 시상식 화제성을 `주파수`·
    `안테나`·`송신`으로 비유하는 연예 기사 관용구가 바레 `주파수` 강한
    마커와 충돌해 그대로 통과하던 문제. 시상식 맥락 키워드(청룡·백상·BSA·
    어워즈 등)와 근접 공기할 때만 매칭하도록 좁게 설계.
  - `product_launch_hype`(우선순위 패턴 신규): "삼성, 갤럭시 탭 S12·S26 FE
    출격 임박…국내 전파인증 완료" — 제품 출시 마케팅 기사가 `전파인증`
    (0.4.17.0에서 추가한 정당한 도메인 복합어)을 부속 정보로 인용해 강한
    마커와 충돌. "출격 임박" 등 마케팅 클리셰 문구로 판정.
  - `culture_entertainment`에 `기상캐스터`·`쇼핑호스트`·`옷입고` 추가:
    "저런 옷입고 방송? 화제의 기상캐스터 누구" — 강한 마커 없이 순수
    커버리지 공백.
  - `geopolitics_trade`에 `격추`·`공습`·`드론 공격`·`무인기 공격`·
    `미사일 발사` 추가: "이란 방송, 이라크 서부서 미군 드론 격추" —
    기존 패턴엔 "전쟁·분쟁·침공" 등만 있고 개별 군사행위 용어가 없었음.
  - 정상 ICT 기사(전파인증 제도 개편, 6G 주파수 실증, ITU-R 표준 논의,
    UHD 안테나 송출 등) 5건은 우선순위 패턴 문구 자체가 없어 여전히 통과 —
    신규 테스트로 확인.
  - `tests/test_collect_filter.py`에 재현 4건 + 정상 ICT 회귀 방지 4건
    추가(총 174개 통과, 신규 8건).
